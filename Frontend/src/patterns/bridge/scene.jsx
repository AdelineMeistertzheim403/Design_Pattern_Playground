...
export default function BridgeScene({
  ...
  const model = extractBridgeModel(execution)
  ...
  const playback = useScenePlayback(
    useMemo(() => buildPlaybackFrames(model.steps, 'Bridge ready'), [model.steps]),
    900,
  )
    ...
    <div className={panelClassName}>
      ...
      <ZoomableViewport enabled={false} viewportClassName={isExpanded ? 'mt-6' : 'mt-4'}>
        <svg className={svgClassName} viewBox={`0 0 ${viewBoxWidth} ${viewBoxHeight}`} role="img">
          ...
          <text x={timelineX + 24} y={timelineY + 60} fontSize="24" fontWeight="700" fill="#241f18">{model.stepCount} etape(s)</text>
