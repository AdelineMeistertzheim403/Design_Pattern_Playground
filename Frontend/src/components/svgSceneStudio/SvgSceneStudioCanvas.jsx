import { buildArrowPath, getElementCenter, sanitizeRawMarkup, TONES } from './svgSceneStudioDocument'
import { buildSceneGridPattern } from './SvgSceneStudioPalette'

export default function SvgSceneStudioCanvas({
  draft,
  ...
  return (
    <section className="rounded-[34px] border border-black/10 bg-[linear-gradient(180deg,rgba(255,250,242,0.99),rgba(247,240,226,0.94))] p-4 shadow-[0_18px_45px_rgba(47,37,22,0.08)]">
      <div className="overflow-auto rounded-[24px] border border-black/10 bg-[#fffaf2]">
        <svg ref={svgRef} className="block h-auto min-w-full" viewBox={draft.viewBox} onPointerDown={onBackgroundPointerDown}>
          <defs>
            <marker id="svg-scene-studio-arrow" markerWidth="10" markerHeight="10" refX="9" refY="5" orient="auto">
              <path d="M 0 0 L 10 5 L 0 10 z" fill="#7a5a3f" />
            </marker>
            {buildSceneGridPattern()}
          </defs>
          <rect width={viewBox.width} height={viewBox.height} rx="32" fill="#fffaf2" />
          {showGrid ? <rect width={viewBox.width} height={viewBox.height} rx="32" fill="url(#svg-scene-studio-grid)" /> : null}
            ...
            return (
              <g key={element.id} className="cursor-pointer" onPointerDown={(event) => onElementSelect(event, element.id)}>
                ...
                {element.type !== 'text' && element.type !== 'raw' ? (
                  <>
                    <text x={center.x} y={element.y + Math.min(56, element.height / 2)} textAnchor="middle" fontSize={element.fontSize ?? 24} fontWeight="700" fill={tone.text} className="pointer-events-none select-none">{element.label}</text>
                    {element.subtitle ? <text x={center.x} y={element.y + Math.min(92, element.height - 18)} textAnchor="middle" fontSize="16" fill={tone.text} className="pointer-events-none select-none">{element.subtitle}</text> : null}
                  </>
                ) : null}
                ...
                {isActive ? (
                  <rect x={element.x + element.width - 16} y={element.y + element.height - 16} width="16" height="16" rx="4" fill="#c25737" className="cursor-se-resize" onPointerDown={(event) => onElementResizeStart(event, element)} />
                ) : null}
              </g>
            )
          })}
          ...
          {draft.arrows.map((arrow) => {
            ...
            return (
              <g key={arrow.id}>
                <path d={buildArrowPath(arrow)} fill="none" stroke="transparent" strokeWidth="22" className="cursor-pointer" onPointerDown={(event) => onArrowSelect(event, arrow.id)} />
                <path d={buildArrowPath(arrow)} fill="none" stroke={isActive ? '#c25737' : '#7a5a3f'} strokeWidth={isActive ? '6' : '4'} strokeDasharray={arrow.dashed ? '12 10' : '0'} markerEnd="url(#svg-scene-studio-arrow)" pointerEvents="none" />
                {(draft.playbackMode ?? 'auto') === 'auto' && arrow.animation?.enabled !== false ? (
                  <circle r={arrow.animation?.pointRadius ?? 5} fill={arrow.animation?.color ?? '#246b5e'} opacity="0.95" pointerEvents="none">
